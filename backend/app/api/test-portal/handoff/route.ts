...
export async function POST(request: NextRequest) {
  ...
  if (!secretText || secretText.length < 32) return json({ error: 'Portal handoff is not configured.' }, 503);
  const token = request.headers.get('authorization')?.replace(/^Bearer\s+/i, '').trim();
  if (!token) return json({ error: 'Authentication is required.' }, 401);
  const { data: authData, error: authError } = await supabase.auth.getUser(token);
  if (authError || !authData.user) return json({ error: 'Authentication is invalid or expired.' }, 401);
  const body = await request.json().catch(() => ({}));
  const classroomId = typeof body.classroomId === 'string' ? body.classroomId : '';
  if (!/^[0-9a-f-]{36}$/i.test(classroomId)) return json({ error: 'A valid classroomId is required.' }, 400);
  ...
  if (!owned?.length && !enrolled?.length) return json({ error: 'You are not authorized for this classroom.' }, 403);
  ...
  return json({ handoffToken, expiresInSeconds: 60 });
}
