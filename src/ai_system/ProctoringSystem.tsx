...
import { Shield, AlertTriangle, Eye, Activity, Play, Square, Video } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { LocalMLService } from '../services/ml/LocalMLService';
import { CameraService } from '../services/ml/CameraService';
import { logMalpracticeDetected } from '../lib/analytics';
...
export const ProctoringSystem: React.FC = () => {
  ...
  const [logs, setLogs] = useState<{ time: string, status: 'Flagged' | 'Clean', reason: string, type: string, confidence: number }[]>([]);
  ...
  const [currentViolation, setCurrentViolation] = useState<{ reason: string, box: any } | null>(null);
